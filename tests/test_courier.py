...
class TestCourier:
    ...
    def test_login_success(self, created_courier):
        ...
        with allure.step("Выполняем запрос авторизации"):
            json_response, status_code, courier_id = courier_methods.login_courier(
                auth_data
            )
    ...
    def test_create_duplicate_courier(self, courier_data):
        with allure.step("Создаём курьера первый раз (успешно)"):
            courier_methods = CourierMethods()
            _, _ = courier_methods.create_courier(courier_data)
    ...
    def test_create_courier_missing_fields(self, courier_data, missing_field):
    ...
    def test_login_wrong_credentials(self, courier_data, wrong_field, wrong_value):
    ...
    def test_login_missing_login(self, courier_data):
        ...
        with allure.step("Выполняем запрос авторизации без логина"):
            courier_methods = CourierMethods()
            json_response, status_code, _ = courier_methods.login_courier(
                incomplete_data
            )
    ...
    def test_login_missing_password(self, courier_data):
        ...
        with allure.step("Выполняем запрос авторизации без пароля и ожидаем таймаут"):
            with pytest.raises(requests.exceptions.ReadTimeout):
                courier_methods.login_courier(incomplete_data)
